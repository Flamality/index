...
export default function SpotifyController() {
  ...
  useEffect(() => {
    ...
    fetchSong();
    interval = setInterval(fetchSong, 4000);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("spotifyControllerMinimized", minimized);
  }, [minimized]);

  useEffect(() => {
    const savedState = localStorage.getItem("spotifyControllerMinimized");
    if (savedState !== null) {
